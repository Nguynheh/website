...
<table class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $item->slug }}</td>
            <td>
                <a href="{{ route('admin.noidung_phancong.edit', $item->id) }}" class="btn btn-sm btn-warning">Sửa</a>
                <form action="{{ route('admin.noidung_phancong.destroy', $item->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
