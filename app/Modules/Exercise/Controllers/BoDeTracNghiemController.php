...
class BoDeTracNghiemController extends Controller
{
    ...
    // Phương thức hiển thị trang làm bài trắc nghiệm
public function startQuiz($id)
{
    $bodeTracNghiem = BoDeTracNghiem::findOrFail($id);
    $questions = json_decode($bodeTracNghiem->questions, true); // Lấy câu hỏi từ bộ đề
    if (empty($questions)) {
        return redirect()->route('frontend.hoctap.index')->with('error', 'Bộ đề trắc nghiệm không có câu hỏi.');
    }
    $active_menu = 'bode_tracnghiem_start';

    return view('frontend.hoctap.start', compact('bodeTracNghiem', 'questions', 'active_menu'));
}

// Phương thức nộp bài trắc nghiệm và tính điểm
public function submitQuiz(Request $request, $id)
{
    ...
    $totalPoints = 0;
    $userAnswers = $request->input('answers', []); // Lấy câu trả lời từ form gửi lên

    // Kiểm tra và tính điểm cho mỗi câu hỏi
    foreach ($questions as $question) {
        if (isset($userAnswers[$question['id_question']])) {
            $answer = $userAnswers[$question['id_question']];
            // Kiểm tra câu trả lời đúng/sai và cộng điểm
    ...
    // Lưu kết quả vào cơ sở dữ liệu (có thể tạo bảng để lưu lịch sử làm bài của người dùng)
    // Ví dụ: Lưu điểm của người dùng
    $user = auth()->user();
    $user->results()->create([
        'bode_tracnghiem_id' => $bodeTracNghiem->id,
        'score' => $totalPoints,
        'user_id' => $user->id,
    ]);

    // Redirect và thông báo kết quả
    return redirect()->route('admin.bode_tracnghiem.index')->with('success', 'Bài trắc nghiệm đã được nộp thành công với điểm: ' . $totalPoints);
}
