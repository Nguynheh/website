                    ...
                    <tbody>
                        @foreach($phancongs as $phancong)
                            <tr>
                                ...
                                <td>{{ $phancong->class->class_name ?? 'N/A' }}</td> <!-- HIỂN THỊ LỚP -->
                                <td>{{ $phancong->hocky->so_hoc_ky }}</td>
                                <td>{{ $phancong->namhoc->nam_hoc }}</td>
                                <td>{{ \Carbon\Carbon::parse($phancong->ngayphancong)->format('d-m-Y') }}</td>
                                <td>{{ $phancong->time_start ? \Carbon\Carbon::parse($phancong->time_start)->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $phancong->time_end ? \Carbon\Carbon::parse($phancong->time_end)->format('d-m-Y') : 'N/A' }}</td>
                                <td>
                                    <!-- Chỉnh sửa -->
